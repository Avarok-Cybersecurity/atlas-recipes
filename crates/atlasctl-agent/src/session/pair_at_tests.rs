...
#[test]
fn pairing_at_an_address_reports_the_identity_that_answered() {
    let f = Fixture::new();
...
#[test]
fn an_address_paired_exchange_is_confirmed_the_same_way() {
    let f = Fixture::new();
    let node = NodeId::from_bytes([7; 32]);
    let fleet = RecordingFleet::new(node);
    let mut s = session(&f, &fleet);
...
/// never reached.
#[test]
fn an_address_that_answers_nothing_names_no_node() {
    let f = Fixture::new();
    let node = NodeId::from_bytes([7; 32]);
    let fleet = RecordingFleet::new(node);
    fleet.start_failing();
...
/// with live words.
#[test]
fn a_failed_attempt_spends_any_exchange_already_pending() {
    let f = Fixture::new();
    let node = NodeId::from_bytes([7; 32]);
    let fleet = RecordingFleet::new(node);
    let mut s = session(&f, &fleet);

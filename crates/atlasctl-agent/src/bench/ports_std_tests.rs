...
fn the_verdict_line_is_parsed_exactly_as_atlas_prints_it() {
    ...
    assert!(parse_verdict("2026-09-13 INFO spark: ready").is_none());
...
fn a_cached_binary_is_a_hit_only_when_provenance_and_bytes_agree() {
    ...
    let cfg = BenchConfig {
        atlas_repo: dir.join("repo"),
        atlas_home: dir.join("home"),
        hardware: "gb10".into(),
        env: Default::default(),
    ...
    std::fs::write(bdir.join("spark"), b"binary bytes").unwrap();
    assert_eq!(
        ports.cached_binary(&sha).unwrap(),
        Err(CacheMiss::NoProvenance)
    );

    let (good_hash, bytes) = sha256_file(&bdir.join("spark")).unwrap();
    ...
    std::fs::write(bdir.join("spark"), b"tampered").unwrap();
    assert_eq!(
        ports.cached_binary(&sha).unwrap(),
...
fn a_job_collects_its_own_records_and_not_the_previous_jobs() {
    ...
    let ports = StdPorts::new(cfg);
    let before = ports.record_state(&dir.join("wt"), gate).unwrap();
    assert_eq!(
        before.len(),
        2,
    ...
    let got = ports
        .collect(&dir.join("wt"), gate, &before, &dir.join("dest"))
        .unwrap();
    ...
    let before2 = ports.record_state(&dir.join("wt"), gate).unwrap();
    std::thread::sleep(std::time::Duration::from_millis(20));
    write("2026-09-17-abc-s4of6.json", "{\"shard\":4,\"rerun\":true}");
    let again = ports
        .collect(&dir.join("wt"), gate, &before2, &dir.join("dest2"))
        .unwrap();
    assert_eq!(
    ...
    let before3 = ports.record_state(&dir.join("wt"), gate).unwrap();
    assert!(
        ports
    ...
    let before4 = ports.record_state(&dir.join("wt"), gate).unwrap();
